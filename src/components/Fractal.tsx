...
import { Mandlebrot, MandlebrotStruct, iMandlebrot } from "../Mandlebrot";
...
export const Fractal: React.FC<Props> = (props) => {
  const mandlebrot: iMandlebrot = Mandlebrot();
  ...
  /** EVENTS */
  function onZoomInClicked(event: React.MouseEvent<HTMLButtonElement>) {
    setMandlebrotStruct(mandlebrot.zoom(mandlebrotStruct, 0.5));
  };

  function onZoomOutClicked(event: React.MouseEvent<HTMLButtonElement>) {
  ...
  function onImageLoad(event: React.SyntheticEvent) {
    setImageLoaded(true);
  };

  function onImageClicked(event:  React.MouseEvent<HTMLImageElement>) {
    if (mouseX && mouseY) {
      setMandlebrotStruct(mandlebrot.imageClicked(mandlebrotStruct, mouseX, mouseY));
    }
  };

  function onResetClicked(event: React.MouseEvent<HTMLElement>) {
